...
This dataset contains a total of `r nrow(instacart)` rows and `r ncol(instacart)` columns, with each row resprenting a single product from an instacart order.
...
accel_agg = accel_data %>%
  mutate(daily_activity = sum(minute_1:minute_1440)) %>%
  select(week, day_id, day, type_of_day, daily_activity, everything()) #Reorganize columns so minute_* are at the end
